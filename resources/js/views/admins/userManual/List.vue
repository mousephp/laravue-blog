		...
		<div class="col-xs-12 col-sm-12 col-md-12 col-lg-6 col-xl-12">						
			<div class="card mb-3">
				<div class="card-header">
					<h3><i class="fa fa-table"></i> Thành phần User Manual
						<router-link class="btn btn-primary btn-sm float-right" :to="{ name: 'UserManualCreate' }">Thêm</router-link></h3>
				</div>
				...
				<div class="card-body">
					...
					<table id="example1" class="table table-bordered table-hover display">
						...
						<tbody>
							<tr v-for="(value, index) in user_manuals" :key="value.id">
								<td>{{ value.id }}</td>
								<td>{{ value.title }}</td>
								<!-- <td>{{ value.content }}</td> -->
								<td>
								...
								<td>
									<router-link :to="{ name: 'UserManualEdit', params: {id: value.id }}" class="btn btn-success btn-sm">
	...
	methods:{
		destroy(id, index){
			this.$swal({
				...
				if (result.value) { 
					this.$store.dispatch('deleteUserManual', id);                  
